...
import asyncio 
async def call_company(line):
    try:
        await asyncio.sleep(line['call_time'])
        if line['call_time'] > 8:
            task = asyncio.current_task()
            task.cancel()
        else:
            return f"Company {line['Name']}: {line['Phone']} дозвон успешен"
    except asyncio.CancelledError:
        pass

async def main():
    tasks = [asyncio.create_task(call_company(line)) for line in data]
